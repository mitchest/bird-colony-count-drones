...
var NUM_TREES = 500
var SEED = 42
var NUM_SITES = 5 // out of 18
var NON_NEST_POINTS = 1e4
var REDUCER_SCALE = 0.05
var EXPORT_SCALE = 0.1
var USE_RGB = false

var filename = function (){
  return '/users/mitchest/drone-nest-count/eulimbah_rf_rscale' + 
...
var colony = ee.FeatureCollection("users/mitchest/drone-nest-count/eulimbah_colony_east") //colony boundary

var training_sites = ee.FeatureCollection("users/mitchest/drone-nest-count/eulimbah_training_sites_east") //potnetial image training sites
...
// special case here for missing data - clip the colony boundary by the ortho data, and clip ortho data
var ortho_colony = ortho.select(['b3']).gt(0).reduceToVectors({geometry: colony, scale: 0.1, maxPixels: 1e10})
Map.addLayer(ortho_colony, {}, 'ortho colony clip')
ortho = ortho.clip(ortho_colony)
...
var non_nests = ee.Image(0).toUint16().reduceToVectors({
  geometry: ee.FeatureCollection.randomPoints(ortho_colony, NON_NEST_POINTS, SEED),
  geometryType: 'centroid',
  scale: REDUCER_SCALE
})
...
//Map.addLayer({ eeObject: train_points, name: 'train points' })
...
var train_vectors = train_composite.reduceRegions({
  ...
  scale: 0.1 // 10 cm around a point is probably enough (given nest marking point is variable)
})
// filter out any pesky points that still managed to slip in
//print(train_vectors.size())
train_vectors = train_vectors.filter(ee.Filter.neq('white', null))
//Map.addLayer({ eeObject: train_vectors, name: 'train vectors' })
//print(train_vectors.first())
//print(train_vectors.filter(ee.Filter.eq("system:index", '1_+321031280-76918767')).first())
//print(train_vectors.size())
...
var export_img = function () {
  Export.image.toAsset({
    image: bird_nests,
    description: 'eulimbah_east',
    assetId: OUTPUT_FILE,
    scale: EXPORT_SCALE,
    region: ortho_colony, // this needs to be updated too!
    maxPixels: 1e10
  })
}
