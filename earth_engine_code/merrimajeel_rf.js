// Lower Lachlan River, upper Merrimajeel creek
// Table of Contents
...
var filename = function (){
  return '/users/mitchest/drone-nest-count/merrimajeel_rf_rscale' + 
     REDUCER_SCALE*100 + '_exp_scale' + EXPORT_SCALE*100 + '_sites' + NUM_SITES
}
var OUTPUT_FILE = filename()

//'users/mitchest/drone-nest-count/merrimajeel_training_sites'
// 1. Loading the data
// load image data
var ortho = ee.Image("users/mitchest/drone-nest-count/booligal_4cm_ortho")
// dsm's are a good choice if features of interest are distinct, in this case there's too much lignum that 'looks' like nests
// var dsm = ee.Image("users/mitchest/drone-nest-count/booligal_DSM")

// load nest and other feature data
var nests = ee.FeatureCollection('ft:16FUGbTIh7M0f-wcYyes6CsbPdJxPMNiNgvyAMWn3') //identified nests
var colony = ee.FeatureCollection('ft:1xQVSam3FK8WwZfqQ9CRK3bTCyR2WgMibdj5IwoDR') //colony boundary

var training_sites = ee.FeatureCollection("users/mitchest/drone-nest-count/merrimajeel_training_sites") //potnetial image training sites
...
/*
train_vectors.classify(classifier).errorMatrix('label', 'classification').accuracy()
             .aside(print)
var cm = classifier.confusionMatrix()
*/

//  export the classification as an asset for exploration w/ javascript

var bird_nests = train_composite.classify(classifier)

//var old = ee.Image('users/JohnHWilshire/bird_nests')
//Map.addLayer(old, {min: 0, max: 1, palette: ['#000','#0f0']}, 'old')

//var new_ = ee.Image('users/JohnHWilshire/bird_nests_2')

//Map.addLayer(bird_nests.mask(bird_nests.eq(1)), {min: 0, max: 1, palette: '000000,00ff00'}, 'new')
